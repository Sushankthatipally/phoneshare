...
async function handleRequest(req, res) {
  ...
  if (req.method === 'GET' && pathname.startsWith('/api/guest/')) {
    ...
    // JSON share state for the desktop UI — does NOT consume a page-load use.
    if (parts.length === 5 && parts[4] === 'state') {
      const token = decodeURIComponent(parts[3]);
      const share = store.getGuestShare(token);
      if (!share) return sendJson(res, 404, { ok: false, error: 'Share not found or expired' });
      const { files, ...rest } = share;
      return sendJson(res, 200, {
        ok: true,
        share: { ...rest, files: files.map(({ storagePath, ...file }) => file) },
      });
    }
  }
